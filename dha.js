...
const os = require('os');
const absen = JSON.parse(fs.readFileSync('./database/group/absen.json'))
	...
	try {
     ...
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
       ...
       console.error(err)
        ...
        const ftoko = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync('./media/ganteng.jpg')},"title": `HALLO${pushname}},"businessOwnerJid": `0@s.whatsapp.net`}}}
...
`
               ...
               buttonsMessage = {
               contentText: `${menu}`,
               footerText: '𝐀𝐇𝐌𝐀𝐃 𝐑𝐀𝐅𝐋𝐈 𝐘𝐔𝐃𝐈𝐊𝐀', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
[ ${args[0]} ] Ketik Y/N untuk menerima atau menolak permainan`
        ...
        case 'resetgame':
               if (!isGroup) return reply(mess.only.group)
               if (!isTTT) return reply('Tidak ada permainan di group ini')
        ...
        case 'ttt':
              ...
              if (isTTT) return reply('Sedang ada permainan di group ini, harap tunggu')
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yang Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
              gameAdd(sender, glimit)
              break
       case 'family100':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
       ...
       case 'tebakanime':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (tebakanime.hasOwnProperty(sender.split('@')[0])) return reply("Selesein yang sebelumnya dulu atuh")
       ...
       case 'tebaklagu':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (tebaklagu.hasOwnProperty(sender.split('@')[0])) return reply("Selesein yang sebelumnya dulu atuh")
       ...
       case 'tebaktebakan':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (tebaktebakan.hasOwnProperty(sender.split('@')[0])) return reply("Masih ada soal yang belum terjawab")
       ...
       case 'kuismath':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (kuismath.hasOwnProperty(sender.split('@')[0])) return reply("Selesein yang sebelumnya dulu atuh")
              ...
              break
       case 'tebakgambar':
              if (tebakgambar.hasOwnProperty(sender.split('@')[0])) return reply("Selesein yang sebelumnya dulu atuh")
       ...
       case 'siapaaku':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (tebaksiapaaku.hasOwnProperty(sender.split('@')[0])) return reply("Masih ada soal yang belum terjawab")
       ...
       case 'tebakkata':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (tebakata.hasOwnProperty(sender.split('@')[0])) return reply("Masih ada soal yang belum terjawab")
       ...
       case 'tebaklirik':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (tebaklirik.hasOwnProperty(sender.split('@')[0])) return reply("Masih ada soal yang belum terjawab")
      ...
      case 'tebakjenaka':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (tebakjenaka.hasOwnProperty(sender.split('@')[0])) return reply("Masih ada soal yang belum terjawab")
       ...
       case 'kimiakuis':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (tebakimia.hasOwnProperty(sender.split('@')[0])) return reply("Masih ada soal yang belum terjawab")
       ...
       case 'tebakbendera':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (tebakbendera.hasOwnProperty(sender.split('@')[0])) return reply("Masih ada soal yang belum terjawab")
       ...
       case 'susunkata':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (susunkata.hasOwnProperty(sender.split('@')[0])) return reply("Masih ada soal yang belum terjawab")
       ...
       case 'asahotak':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (asahotak.hasOwnProperty(sender.split('@')[0])) return reply("Masih ada soal yang belum terjawab")
       ...
       case 'caklontong':
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (caklontong.hasOwnProperty(sender.split('@')[0])) return reply("Masih ada soal yang belum terjawab")
...
• *140 DIAMOND = Rp 19.200*
• *210 DIAMOND = Rp 28.950*
• *280 DIAMOND = Rp 38.450*
• *355 DIAMOND = Rp 48.850*
• *425 DIAMOND = Rp 58.500*
• *510 DIAMOND = Rp 68.550*
• *720 DIAMOND = Rp 95.400*
• *1440 DIAMOND = Rp 185.350*
• *2140 DIAMOND = Rp 270.350*
• *M.MINGGUAN = Rp 28.500*
• *M.BULANAN = Rp 140.000*
━━━━━━━━━━━━━━━━━━━━
*NOTE :*
...
• *257 DIAMOND = Rp 58.000*
• *344 DIAMOND = Rp 77.000*
• *429 DIAMOND = Rp 95.500*
• *514 DIAMOND = Rp 115.000*
• *706 DIAMOND = Rp 153.450*
• *878 DIAMOND = Rp 191.000*
• *1412 DIAMOND = Rp 305.300*
• *2195 DIAMOND = Rp 455.600*
• *STARTLIGHT M = Rp 127.000*
• *TWILIGHT PASS = Rp 127.000*
━━━━━━━━━━━━━━━━━━━━
*NOTE :*
_HARGA BISA BERUBAH_
_SEWAKTU WAKTU_
...
case 'tiktok':
              ...
              imageMsg = (await dha.prepareMessageMedia(fs.readFileSync(`./media/yudha.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/yudha.jpg`)})).imageMessage
              buttonsMessage = {footerText:'Jangan Lupa Subscribe Yt RCC STORE\n Helpme tu 1k subscriber', imageMessage: imageMsg,
              contentText:`Silahkan pilihan media yang mau di download kak:v`,buttons,headerType:4}
...
v = await fetchJson(`https://api.waifu.pics/nsfw/waifu`)
...
break
       case 'husbu':
              getBuffer(`https://api.lolhuman.xyz/api/random/husbu?apikey=${setting.lolkey}`).then((gambar) => {
              dha.sendMessage(from, gambar, image, { quoted: mek })
})
break
       case 'milf':
              getBuffer(`https://api.lolhuman.xyz/api/random/milf?apikey=${setting.lolkey}`).then((gambar) => {
              dha.sendMessage(from, gambar, image, { quoted: mek })
})
break
       case 'cosplay':
       ...
       case 'img2url':
         ...
         case 'asupan': // shansekai                
              ...
              .then(async (body) => {
               ...
               sendMediaURL(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, '\`\`\`ASUPAN BRO GUA DAPAT DARI LORD ARFYUDIKA\`\`\`')
...
if (args.length == 0) return reply(`Idnya mana kak?`)
                    ff_id = args[0]
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/freefire/${ff_id}?apikey=rccstore`)
                    reply(`nih kak`)
                    break      
//------------------< Sticker/Tools >-------------------
      ...
      case 'doge':
       ...
       case 'patrick':
             ...
             .then(body => {
              ...
              break
       case 'gura':
              getBuffer(`https://api.lolhuman.xyz/api/sticker/gawrgura?apikey=${setting.lolkey}`).then((gambar) => {
              dha.sendMessage(from, gambar, image, { quoted: mek })
}) 
       case 'gawrgura':
              reply(mess.wait)
...
break
       ...
       case 'ttg':
              ...
              reply(mess.wait)
              sendWebp(from, `https://api.vhtear.com/textxgif?text=${q}&apikey=NOT-PREMIUM${setting.vhtearkey}`)
       ...
       case 'randombokep':
              ...
              break
                case 'xnxx':
                    ...
                    query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/xnxx?apikey=rccstore&url=${query}`)
                    get_result = get_result.result
                    ini_txt = `Title : ${get_result.title}\n`
                    ini_txt += `Duration : ${get_result.duration}\n`
...
dha.sendMessage(from, tapib1, document, { quoted: mek, filename:`RCC STORE ~ 404 ${vipi}.mp3`, mimetype: 'audio/application' })
...
"thumbnail": "",
"itemCount": 1000000000000,
"status": "INQUIRY",
"surface": "CATALOG",
"orderTitle": "GUA GK NGERTI",
"message":"entah lah gua nob",
"sellerJid": "6285282609948@s.whatsapp.net",
...
"title": `RCC STORE ~ 404 ${vipi}`,
"description": `${virtex}`,
"currencyCode": "IDR",
"priceAmount1000": "99999999999999999999999999999999",
"productImageCount": 1
},
"businessOwnerJid": "6285282609948@s.whatsapp.net",
"contextInfo": {
"forwardingScore": 3,
"isForwarded": true
}
...
case 'getvn':
      ...
      case 'broadcast':
             ...
             for (let _ of anu) {
             sendMess(_.jid, `*「 PESAN SIARAN RCC STORE」*\n\n${body.slice(4)}`)
}
             reply('Suksess broadcast')
}
             break
...
//------------------< Fun >-------------------
    case 'public':
       ...
       case 'kapankah':
              ...
              break
       case 'truth':
              const trut =['Pernah suka sama siapa aja? berapa lama?','Kalau boleh atau kalau mau, di gc/luar gc siapa yang akan kamu jadikan sahabat?(boleh beda/sma jenis)','apa ketakutan terbesar kamu?','pernah suka sama orang dan merasa orang itu suka sama kamu juga?','Siapa nama mantan pacar teman mu yang pernah kamu sukai diam diam?','pernah gak nyuri uang nyokap atau bokap? Alesanya?','hal yang bikin seneng pas lu lagi sedih apa','pernah cinta bertepuk sebelah tangan? kalo pernah sama siapa? rasanya gimana brou?','pernah jadi selingkuhan orang?','hal yang paling ditakutin','siapa orang yang paling berpengaruh kepada kehidupanmu','hal membanggakan apa yang kamu dapatkan di tahun ini','siapa orang yang bisa membuatmu sange','siapa orang yang pernah buatmu sange','(bgi yang muslim) pernah ga solat seharian?','Siapa yang paling mendekati tipe pasangan idealmu di sini','suka mabar(main bareng)sama siapa?','pernah nolak orang? alasannya kenapa?','Sebutkan kejadian yang bikin kamu sakit hati yang masih di inget','pencapaian yang udah didapet apa aja ditahun ini?','kebiasaan terburuk lo pas di sekolah apa?']
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'cekapikey':
              ...
              anu = await fetchJson(`https://api.lolhuman.xyz/api/checkapikey?apikey=rccstore${q}`)
       ...
       case 'bugreport':
              ...
              dha.sendMessage('6285282609948@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
...
case 'tahta':
         ...
         buffer = await getBuffer(`https://api.lolhuman.xyz/api/hartatahta?apikey=rccstore&text=${teks}`) 
         dha.sendMessage(from, buffer, image, {quoted: ftoko, caption : 'ini hasilnya kak....'})
         break    
case 'pornhub':
         if (args.length == 0) return reply(`Contoh: ${prefix + command} RCC STORE`)
         txt1 = args[0]
         txt2 = args[1]
         ini_anu = await getBuffer(`http://api.lolhuman.xyz/api/textprome2/pornhub?apikey=rccstore&text1=${txt1}&text2=${txt2}`)
...
//------------------< Menunya Bang:v >-------------------
      case 'dana':
             reply(`DANA : 081221661191\n\nYuliani`)
             break
           case 'gopay':
             reply(`GOPAY : 085282609948\n\nEdi Mulyadi`)
             break  
         case 'pulsa':
             reply(`PULSA : 085282609948\n\nTelkomsel`)
             break              
      case 'infoig':
             reply(`Follow Instagram Owner\ninstagram.com/arfyudika`)
             break
      case 'grupbot':
             reply('https://chat.whatsapp.com/ICe8hTrQqvHHlp5YcsiazZ')
             break
      case 'ownermenu':
